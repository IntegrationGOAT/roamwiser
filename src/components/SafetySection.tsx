import type { RiskData } from '../services/openrouter.ts'
import { ShieldIcon, WeatherIcon, AlertIcon } from './Icons'
      ...
      {isGenerating ? (
        ...
        <div className="safety-grid">
          ...
          <div 
            ...
            onClick={() => onToggleCard('weather-forecast')}
          >
            <WeatherIcon />
            ...
            <ul className="safety-list">
              {riskData.weather.slice(0, 2).map((tip, idx) => (
                <li key={idx} className="safety-list-item">• {tip}</li>
              ))}
            </ul>
              ...
              <div className="weather-forecast">
                ...
                {riskData.weatherForecast.map((forecast, idx) => (
                  <div key={idx} className="forecast-day">
                    ...
                    <div className="forecast-detail"><strong>Condition:</strong> {forecast.condition}</div>
                    <div className="forecast-detail"><strong>Temp:</strong> {forecast.temperature}</div>
                    <div className="forecast-detail"><strong>Humidity:</strong> {forecast.humidity}</div>
          ...
          <div className="safety-card">
            ...
            <ul className="safety-list">
              {riskData.advisories.map((advisory, idx) => (
                <li key={idx} className="safety-list-item">• {advisory}</li>
              ))}
            </ul>
          </div>
        ...
        <div className="empty-state">
          ...
          <p className="empty-state-text">Generate your itinerary to get personalized safety advisories and weather forecasts</p>
        </div>
      )}
    </section>
  )
